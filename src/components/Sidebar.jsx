    ...
    <>
      {mobileOpen && (
        <div
          ...
          className="fixed inset-0 bg-[#252321]/30 z-40 lg:hidden"
        ></div>
      )}

      <aside className={`fixed top-0 left-0 bottom-0 z-50 w-72 bg-[var(--bg-surface)] border-r border-[var(--border-soft)] flex flex-col justify-between transition-transform duration-200 lg:translate-x-0 ${
        ...
        <div className="p-5 space-y-6 flex-1 overflow-y-auto">
          <div className="flex items-center justify-between pb-3 border-b border-[var(--border-soft)]">
            <div className="flex items-center space-x-3 cursor-pointer" onClick={() => onTabChange('studio')}>
              <div className="w-10 h-10 rounded-md border border-[var(--border-soft)] bg-[var(--bg-secondary)] flex items-center justify-center text-[var(--dark-accent)]">
                <Wrench className="w-4 h-4" />
              </div>
              <div>
                <div className="flex items-center space-x-1">
                  <span className="font-semibold text-[15px] tracking-[-0.02em] text-[var(--text-primary)]">
                    REPAIR
                  </span>
                  <span className="font-medium text-[var(--text-secondary)] text-[11px] uppercase tracking-[0.24em]">
                    LENS
                  </span>
                </div>
              </div>
            ...
            <button
              onClick={onCloseMobile}
              className="lg:hidden p-1 rounded-md text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 bg-[var(--bg-primary)] border border-[var(--border-soft)] text-[10px] text-[var(--text-primary)] placeholder:text-[var(--text-muted)] uppercase tracking-[0.16em] focus:outline-none focus:border-[var(--accent)] transition-all"
            />
          </div>
                    ...
                    categoryItem
                      ? `flex items-center justify-between gap-3 rounded-md border px-3 py-3 ${
                          isActive
                            ...
                            : 'border-transparent text-[var(--text-secondary)] hover:border-[var(--border-soft)] hover:bg-[var(--bg-secondary)]'
                        }`
                      : `flex items-center justify-between px-2.5 py-2.5 rounded-md text-[11px] font-medium uppercase tracking-[0.14em] ${
                          isActive
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className={`flex items-center justify-center rounded-md border ${
                      categoryItem
                        ? isActive
                          ? 'w-9 h-9 border-[var(--border-strong)] bg-[var(--bg-surface)] text-[var(--text-primary)]'
                          : 'w-9 h-9 border-[var(--border-soft)] bg-[var(--bg-secondary)] text-[var(--text-secondary)] group-hover:border-[var(--border-strong)]'
        ...
        <div className="p-4 border-t border-[var(--border-soft)] bg-[var(--bg-secondary)]">
          <div className="flex items-center justify-between p-2.5 bg-[var(--bg-surface)] border border-[var(--border-soft)]">
            <div className="flex items-center space-x-2.5 overflow-hidden">
              <div className="w-8 h-8 rounded-full bg-[var(--bg-secondary)] border border-[var(--border-soft)] flex items-center justify-center font-semibold text-[var(--text-primary)] text-[10px]">
                YS
              </div>
